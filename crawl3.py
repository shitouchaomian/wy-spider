...
#crawl3 继续爬虫每个商品的url,准备crawl4爬取每件商品的好评


'恢复crawl的dict'
ff = webdriver.Firefox()

pandas_dict = from_pickle('./pickle1/crawl2.pkl')
...
def get_type(url,xpath1,turning = False):
    ...
    result_list = []
    for each in items:
        href = each.find_element_by_xpath('../../a').get_attribute('href')
...
def host_sp(topic_to_url,pandas_dict,xpath1):
    ...
    for each in topic_to_url:
        ...
        pandas_dict[each]['url'] = pd.DataFrame(result_list)
        
    return pandas_dict
pandas_dict = host_sp(topic_to_url,pandas_dict,xpath1)

to_pickle(pandas_dict,'./pickle1/crawl3.pkl')
ff.quit()
'传入dict,规整全局数据'
